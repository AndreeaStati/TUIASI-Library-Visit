...
import type { BlockedDateTime, DateTime } from "../../types/calendar";
...
function Calendar({
  ...
  function isDateBlocked(date: number) {
    ...
    if (!blockedForDate || blockedForDate.length === 0) return false;
  ...
  function isHourBlocked(date: number, hour: number) {
    ...
    console.log("Blocked hours:", blockedDates);
  ...
  const dates = Array(startDayOffset)
    .fill(null)
  ...
  useEffect(() => {
    if (
      ...
      const endHour = 11 + parseInt(selectedEndHour);
  ...
  function getDayColor(date: number): string | undefined {
    ...
    if (!slotsForDay || slotsForDay.length === 0) return undefined;

    const hasGray = slotsForDay.some((slot) => slot.color === "gray");
    if (hasGray) return "gray";
      ...
      <Card.Header>
        ...
        <Card.Description>
          <Text as="span" color="yellow.500">
            *Există deja rezervări în această zi, dar mai sunt locuri
            disponibile
          </Text>
        </Card.Description>
      </Card.Header>
      <Card.Body gap="10px">
        ...
        <SimpleGrid columns={7} gap="10px">
          {dates.map((date, index) => {
            ...
            // calculeaza ziua sapt pentru data curenta (0=Sunday, 6=Saturday)
            ...
            // marcare weekend: sambata(6), duminica(0)
        ...
        <Field.Root hidden={selectedDate === ""}>
          <Field.Label>Selecteaza ora la care doresti sa incepi</Field.Label>
          <NativeSelect.Root>
            <NativeSelect.Field
              onChange={(val) => {
                setSelectedStartHour(val.currentTarget.value);
                setSelectedEndHour("");
              }}
              placeholder="Selecteaza ora"
            >
              <option value="1" disabled={isHourBlocked(selectedDay, 11)}>
                11:00
              </option>
              <option value="2" disabled={isHourBlocked(selectedDay, 12)}>
                12:00
              </option>
              <option value="3" disabled={isHourBlocked(selectedDay, 13)}>
                13:00
              </option>
              <option value="4" disabled={isHourBlocked(selectedDay, 14)}>
                14:00
              </option>
            </NativeSelect.Field>
        ...
        <Field.Root hidden={selectedDate === ""}>
          <Field.Label>Selecteaza ora la care doresti sa termini.</Field.Label>
          <NativeSelect.Root>
            <NativeSelect.Field
            ...
            >
              <option
                value="1"
                disabled={
                  parseInt(selectedStartHour) > 1 ||
                  isHourBlocked(selectedDay, 11)
                }
              >
                12:00
              </option>
              <option
                value="2"
                disabled={
                  parseInt(selectedStartHour) > 2 ||
                  isHourBlocked(selectedDay, 12)
                }
              >
                13:00
              </option>
              <option
                value="3"
                disabled={
                  parseInt(selectedStartHour) > 3 ||
                  isHourBlocked(selectedDay, 13)
                }
              >
                14:00
              </option>
              <option
                value="4"
                disabled={
                  parseInt(selectedStartHour) > 4 ||
                  isHourBlocked(selectedDay, 14)
                }
              >
                15:00
              </option>
