...
function Home({optionalButton}: HomeProps) {
    ...
    useEffect(() => {
        const savedTranslationEN = localStorage.getItem("translationEN");
        const savedTranslationRO = localStorage.getItem("translationRO");
        let savedLang = localStorage.getItem("lang");
        if(savedLang === null)
            savedLang = "ro";
        if (savedLang === "ro" && savedTranslationRO) {
            setTranslation(JSON.parse(savedTranslationRO));
            console.log("Loaded translations");
        } else if((savedLang === "en" && savedTranslationEN)){
            console.log("Loaded translations");
          setTranslation(JSON.parse(savedTranslationEN));
        } else {
          loadTranslations().then((data) => {
            setTranslation(data);
            localStorage.setItem(`translation${savedLang.toUpperCase()}`, JSON.stringify(data));
          });
        }
      }, []);
